...
export function IncomeTrend({ transactions }: IncomeTrendProps) {
  const { chartData, currentMonthIncome, lastMonthIncome, percentChange } = useMemo(() => {
    const now = new Date()
    ...
    // Group income by day of month for current and last month
    ...
    transactions.forEach((tx) => {
      // Income is negative amounts or is_income flag
      ...
      if (txMonth === currentMonth && txYear === currentYear) {
        currentMonthDaily[dayOfMonth] = (currentMonthDaily[dayOfMonth] || 0) + amount
        currentMonthTotal += amount
      } else if (txMonth === lastMonth && txYear === lastMonthYear) {
        lastMonthDaily[dayOfMonth] = (lastMonthDaily[dayOfMonth] || 0) + amount
  ...
  // More income = good (green), less income = concerning (red)
  const isIncomeUp = percentChange > 0
  const amountDifference = Math.abs(currentMonthIncome - lastMonthIncome)
    ...
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg">Income This Month</CardTitle>
          <div
            className={`flex items-center gap-1 text-sm ${
              isIncomeUp ? 'text-green-600' : 'text-red-600'
            }`}
          >
            {isIncomeUp ? (
            ...
            <span>
              {Math.abs(percentChange).toFixed(1)}% ({formatCurrency(amountDifference)}) {isIncomeUp ? 'more' : 'less'}
            </span>
          </div>
        </div>
        ...
        <div className="h-[280px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData} margin={{ top: 10, right: 5, bottom: 0, left: 0 }}>
              <defs>
                <linearGradient id="colorLastMonthIncome" x1="0" y1="0" x2="0" y2="1">
